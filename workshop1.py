...
"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  AI THREAT MODELING LEARNING LAB  v2.0                                      ║
║  STRIDE + MAESTRO (Ken Huang / CSA) + OWASP LLM Top 10 (2025)              ║
...
# ══════════════════════════════════════════════════════════════════════
D = {
    "bg":"#0B1622","bg1":"#0F1E2E","bg2":"#152638","bg3":"#1C3050",
    "border":"#243B55","blue":"#2D8CFF","blue2":"#5BA4FF","blue_dk":"#1464CC",
    "amber":"#F5A623","amber2":"#FFD166","green":"#10C98F","red":"#E63950",
    "red2":"#FF6680","purple":"#8B5CF6","purple2":"#A78BFA","teal":"#06B6D4",
    "white":"#EEF2F7","grey":"#7A8BA0","grey2":"#3D526A",
    "L1":"#E63950","L2":"#F5A623","L3":"#EAB308",
    "L4":"#2D8CFF","L5":"#10C98F","L6":"#8B5CF6","L7":"#7A8BA0",
}

st.markdown(f"""<style>
@import url('https://fonts.googleapis.com/css2?family=IBM+Plex+Mono:wght@400;500&family=Sora:wght@300;400;600;700;800&family=Inter:wght@400;500;600&display=swap');
html,body,[class*="css"]{{font-family:'Inter',sans-serif;color:{D['white']};}}
.stApp{{background:{D['bg']};min-height:100vh;}}
.main .block-container{{padding:1.5rem 2rem;max-width:1440px;}}
section[data-testid="stSidebar"]>div{{background:{D['bg1']};border-right:1px solid {D['border']};}}
section[data-testid="stSidebar"] *{{color:{D['white']}!important;}}
h1{{font-family:'Sora',sans-serif;font-weight:800;font-size:2.1rem;
    background:linear-gradient(100deg,{D['blue2']},{D['amber']});
    -webkit-background-clip:text;-webkit-text-fill-color:transparent;margin-bottom:.3rem;}}
h2{{font-family:'Sora',sans-serif;font-weight:700;font-size:1.4rem;color:{D['blue2']};
    border-bottom:1px solid {D['border']};padding-bottom:.35rem;margin-top:1rem;}}
h3{{font-family:'Sora',sans-serif;font-weight:600;font-size:1.1rem;color:{D['white']};}}
h4{{font-family:'Inter',sans-serif;font-weight:600;color:{D['amber2']};}}
.stButton>button{{background:linear-gradient(135deg,{D['blue_dk']},{D['blue']});
    color:white;border:none;border-radius:7px;font-weight:600;font-family:'Inter',sans-serif;
    padding:.45rem .9rem;transition:all .2s;box-shadow:0 2px 10px rgba(45,140,255,.25);width:100%;}}
.stButton>button:hover{{background:linear-gradient(135deg,{D['blue']},{D['blue2']});
    box-shadow:0 4px 18px rgba(45,140,255,.45);transform:translateY(-1px);}}
.stSelectbox>div>div,.stMultiSelect>div>div,
.stTextInput>div>div>input,.stTextArea>div>div>textarea{{
    background:{D['bg2']}!important;border:1px solid {D['border']}!important;
    color:{D['white']}!important;border-radius:7px;}}
.stSlider>div{{color:{D['white']};}}
.stCheckbox>label,.stRadio>div,.stRadio label{{color:{D['white']}!important;}}
[data-testid="metric-container"]{{background:{D['bg2']};border:1px solid {D['border']};
    border-radius:10px;padding:1rem;text-align:center;}}
[data-testid="metric-container"] label{{color:{D['grey']}!important;font-size:.78rem;
    letter-spacing:.5px;text-transform:uppercase;}}
[data-testid="metric-container"] [data-testid="stMetricValue"]{{
    color:{D['blue2']}!important;font-weight:800;font-size:1.7rem;}}
details{{background:{D['bg2']};border:1px solid {D['border']};border-radius:9px;
    padding:.5rem 1rem;margin:.4rem 0;}}
details summary{{color:{D['blue2']};font-weight:600;cursor:pointer;}}
.stProgress>div>div>div>div{{background:linear-gradient(90deg,{D['blue']},{D['green']});border-radius:4px;}}
.stTabs [data-baseweb="tab-list"]{{background:{D['bg2']};border-radius:9px;padding:4px;border:1px solid {D['border']};}}
.stTabs [data-baseweb="tab"]{{color:{D['grey']};border-radius:6px;font-family:'Inter',sans-serif;font-weight:500;}}
.stTabs [aria-selected="true"]{{background:{D['blue_dk']}!important;color:white!important;}}
.stDataFrame{{border-radius:9px;overflow:hidden;}}
.stDataFrame thead tr th{{background:{D['bg1']}!important;color:{D['amber2']}!important;font-weight:700;}}
...
code{{font-family:'IBM Plex Mono',monospace!important;background:{D['bg3']};border-radius:4px;
     padding:2px 7px;color:{D['amber2']};font-size:.84em;}}
.stCaption{{color:{D['grey']}!important;font-size:.8rem;}}
.panel{{background:{D['bg2']};border:1px solid {D['border']};border-radius:10px;
    padding:1.2rem;margin:.5rem 0;box-shadow:0 4px 24px rgba(0,0,0,.25);}}
...
.pill{{display:inline-block;padding:3px 11px;border-radius:20px;font-size:.76rem;
       font-weight:700;font-family:'IBM Plex Mono',monospace;letter-spacing:.3px;}}
.pill-blue{{background:rgba(45,140,255,.2);color:{D['blue2']};border:1px solid {D['blue']};}}
.pill-amber{{background:rgba(245,166,35,.2);color:{D['amber2']};border:1px solid {D['amber']};}}
.pill-green{{background:rgba(16,201,143,.2);color:{D['green']};border:1px solid {D['green']};}}
.pill-red{{background:rgba(230,57,80,.2);color:{D['red2']};border:1px solid {D['red']};}}
.pill-purple{{background:rgba(139,92,246,.2);color:{D['purple2']};border:1px solid {D['purple']};}}
.pill-grey{{background:rgba(122,139,160,.15);color:{D['grey']};border:1px solid {D['grey2']};}}
.pill-teal{{background:rgba(6,182,212,.2);color:{D['teal']};border:1px solid {D['teal']};}}
.alert{{padding:1rem 1.2rem;border-radius:9px;margin:.5rem 0;font-size:.92rem;line-height:1.65;}}
.alert-info{{background:rgba(45,140,255,.1);border:1px solid rgba(45,140,255,.35);}}
.alert-success{{background:rgba(16,201,143,.1);border:1px solid rgba(16,201,143,.35);}}
.alert-warn{{background:rgba(245,166,35,.1);border:1px solid rgba(245,166,35,.35);}}
.alert-danger{{background:rgba(230,57,80,.1);border:1px solid rgba(230,57,80,.35);}}
.alert-maestro{{background:rgba(139,92,246,.1);border:1px solid rgba(139,92,246,.35);}}
.score-box{{padding:1.3rem;border-radius:10px;text-align:center;font-weight:800;font-size:1.1rem;}}
.score-excellent{{background:linear-gradient(135deg,#064F3A,{D['green']});color:white;}}
.score-good{{background:linear-gradient(135deg,#0F3160,{D['blue']});color:white;}}
.score-fair{{background:linear-gradient(135deg,#4A3000,{D['amber']});color:white;}}
.score-poor{{background:linear-gradient(135deg,#3A0A14,{D['red']});color:white;}}
...
ZONES = {
    0: {"name":"Not in Control (External)","color":D["grey"],"fill":"rgba(122,139,160,.08)","stroke":"#3D526A"},
    1: {"name":"Minimal Trust",            "color":D["green"],"fill":"rgba(16,201,143,.08)","stroke":"#0D8862"},
    3: {"name":"Standard Application",    "color":D["amber"],"fill":"rgba(245,166,35,.10)","stroke":"#B87A10"},
    5: {"name":"Elevated Trust",           "color":D["blue"], "fill":"rgba(45,140,255,.10)","stroke":"#1464CC"},
    7: {"name":"Critical",                 "color":D["red"],  "fill":"rgba(230,57,80,.12)","stroke":"#A01830"},
    9: {"name":"Maximum Security (Core)",  "color":D["purple"],"fill":"rgba(139,92,246,.15)","stroke":"#6D28D9"},
}

# ══════════════════════════════════════════════════════════════════════
# ENTERPRISE SVG DIAGRAM ENGINE
# ══════════════════════════════════════════════════════════════════════
def _svg_escape(s):
...
def render_architecture_svg(ws_id, mode="architecture", highlighted_threats=None):
    """
    Render an enterprise-grade SVG architecture diagram.
    mode: 'architecture' | 'zones' | 'threat'
    """
    configs = {
        "1": _ws1_svg_config(),
        "2": _ws2_svg_config(),
...
def _build_svg(cfg, mode, highlighted):
    ...
    lines = [
        ...
        f'style="background:{D["bg2"]};border-radius:12px;font-family:\'Inter\',sans-serif;">'
    ]
    ...
    # Node gradients
    grad_pairs = [
        ("grad_blue",   "#1464CC","#2D8CFF"),
        ("grad_amber",  "#B87A10","#F5A623"),
        ("grad_green",  "#0D8862","#10C98F"),
        ("grad_red",    "#A01830","#E63950"),
        ("grad_purple", "#5B21B6","#8B5CF6"),
        ("grad_teal",   "#0284C7","#06B6D4"),
        ("grad_grey",   "#2D3E52","#3D526A"),
        ("grad_navy",   "#0B1622","#152638"),
    ]
    ...
    # Glow filter
    lines.append('<filter id="glow"><feGaussianBlur stdDeviation="3" result="blur"/>'
                 '<feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge></filter>')
    # Shadow
    lines.append('<filter id="shadow" x="-20%" y="-20%" width="140%" height="140%">'
                 '<feDropShadow dx="0" dy="4" stdDeviation="6" flood-color="rgba(0,0,0,.5)"/></filter>')
    # Threat pulse
    lines.append('<filter id="threat_glow"><feGaussianBlur stdDeviation="5" result="blur"/>'
                 '<feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge></filter>')
    lines.append('</defs>')
    ...
    grid_spacing = 40
    lines.append('<g opacity="0.06">')
    for x in range(0, W, grid_spacing):
        lines.append(f'<line x1="{x}" y1="0" x2="{x}" y2="{H}" stroke="{D["blue"]}" stroke-width=".5"/>')
    for y in range(0, H, grid_spacing):
        lines.append(f'<line x1="0" y1="{y}" x2="{W}" y2="{y}" stroke="{D["blue"]}" stroke-width=".5"/>')
    lines.append('</g>')

    # ── Zone bands (if zones or threat mode) ────────────────────────
    for zb in zones_layout:
        ...
        lines.append(f'<rect x="{x}" y="{y}" width="{w}" height="{h}" rx="8" '
                     f'fill="{fill}" stroke="{stroke}" stroke-width="1.5" stroke-dasharray="6,4"/>')
        lines.append(f'<text x="{x+10}" y="{y+18}" font-size="10" font-weight="700" '
                     f'fill="{stroke}" font-family="IBM Plex Mono,monospace" opacity=".85">{_svg_escape(lbl)}</text>')
    ...
    for tb in cfg.get("trust_boundaries", []):
        ...
        lines.append(f'<line x1="{x1}" y1="{y1}" x2="{x2}" y2="{y2}" '
                     f'stroke="{D["red"]}" stroke-width="2" stroke-dasharray="10,5" opacity=".6"/>')
        mx, my = (x1+x2)//2, (y1+y2)//2
        lines.append(f'<text x="{mx}" y="{my-5}" text-anchor="middle" font-size="9" '
                     f'fill="{D["red"]}" font-weight="700" font-family="IBM Plex Mono,monospace"'
                     f' opacity=".9">{_svg_escape(lbl)}</text>')
    ...
    lines.append('<defs><marker id="arrowhead" markerWidth="8" markerHeight="6" refX="8" refY="3" orient="auto">'
                 ...
                 '<marker id="arrowhead_threat" markerWidth="8" markerHeight="6" refX="8" refY="3" orient="auto">'
                 f'<polygon points="0 0, 8 3, 0 6" fill="{D["red"]}"/></marker></defs>')
    ...
    for edge in edges:
        ...
        marker     = "arrowhead_threat" if is_threat else "arrowhead"
        opacity    = ".9" if is_threat else ".55"
        lbl        = edge.get("label","")
        # Draw slight curve
        mx = (x1+x2)//2
        my = (y1+y2)//2 - 20
        lines.append(f'<path d="M{x1},{y1} Q{mx},{my} {x2},{y2}" fill="none" '
                     f'stroke="{stroke_col}" stroke-width="{stroke_w}" opacity="{opacity}" '
                     f'marker-end="url(#{marker})"/>')
        if lbl:
            lines.append(f'<text x="{mx}" y="{my-5}" text-anchor="middle" font-size="9" '
                         f'fill="{stroke_col}" opacity=".85" font-family="IBM Plex Mono,monospace">'
    ...
    for node in nodes:
        ...
        if shape == "cylinder":
            ...
            lines.append(f'<rect x="{x}" y="{y+ry2}" width="{w}" height="{h-ry2}" '
                         f'fill="url(#{grad_id})" rx="4"/>')
            # Top ellipse
            lines.append(f'<ellipse cx="{cx}" cy="{y+ry2}" rx="{rx2}" ry="{ry2}" '
                         f'fill="url(#{grad_id})"/>')
            # Highlight ellipse
            lines.append(f'<ellipse cx="{cx}" cy="{y+ry2}" rx="{rx2}" ry="{ry2}" '
                         f'fill="none" stroke="{stroke_c}" stroke-width="1.5" opacity=".7"/>')
            lines.append(f'<rect x="{x}" y="{y+ry2}" width="{w}" height="{h-ry2}" '
                         f'fill="none" stroke="{stroke_c}" stroke-width="1.5" opacity=".6" rx="4"/>')
            lines.append('</g>')
        elif shape == "diamond":
            cx = x + w//2; cy = y + h//2
            ...
            border_w = "3" if is_highlighted else "1.5"
            border_c = D["red"] if is_highlighted else stroke_c
            opacity_b = "1" if is_highlighted else ".75"
            lines.append(f'<rect x="{x}" y="{y}" width="{w}" height="{h}" rx="8" '
        ...
        cx = x + w//2; cy = y + h//2
        if icon:
            lines.append(f'<text x="{cx-len(label)*3.2}" y="{cy+2}" font-size="14" '
                         f'dominant-baseline="middle">{icon}</text>')
            lines.append(f'<text x="{cx+10}" y="{cy+1}" text-anchor="middle" '
                         f'font-size="11" font-weight="700" fill="white" '
                         f'dominant-baseline="middle">{_svg_escape(label)}</text>')
        else:
            lines.append(f'<text x="{cx}" y="{cy - (5 if sublabel else 1)}" text-anchor="middle" '
                         f'font-size="11" font-weight="700" fill="white" '
                         f'dominant-baseline="middle">{_svg_escape(label)}</text>')

        if sublabel:
            lines.append(f'<text x="{cx}" y="{cy+12}" text-anchor="middle" '
                         f'font-size="9" fill="rgba(255,255,255,.6)" '
                         f'dominant-baseline="middle">{_svg_escape(sublabel)}</text>')

        # MAESTRO badge
        if maestro and mode != "zones":
            ml_color = D.get(maestro, D["grey"])
            lines.append(f'<rect x="{x+w-32}" y="{y-10}" width="30" height="16" rx="8" '
                         f'fill="{ml_color}" opacity=".95"/>')
            lines.append(f'<text x="{x+w-17}" y="{y-2}" text-anchor="middle" '
                         f'font-size="8" font-weight="800" fill="white" '
                         f'font-family="IBM Plex Mono,monospace">{maestro}</text>')

        # Zone badge (zones mode)
        if mode == "zones" and zone_n > 0:
            zc = ZONES.get(zone_n, ZONES[0])["color"]
            lines.append(f'<rect x="{x}" y="{y-10}" width="26" height="16" rx="8" '
                         f'fill="{zc}" opacity=".9"/>')
            lines.append(f'<text x="{x+13}" y="{y-2}" text-anchor="middle" '
                         f'font-size="8" font-weight="800" fill="white" '
                         f'font-family="IBM Plex Mono,monospace">z{zone_n}</text>')

        # Threat warning badge
        if is_highlighted:
            lines.append(f'<circle cx="{x+w-8}" cy="{y+8}" r="8" fill="{D["red"]}"/>')
            lines.append(f'<text x="{x+w-8}" y="{y+12}" text-anchor="middle" '
                         f'font-size="9" fill="white" font-weight="900">!</text>')

    # ── Title bar ────────────────────────────────────────────────────
    mode_labels = {"architecture":"Architecture Diagram","zones":"Zone Trust Analysis","threat":"Threat Model View"}
    ...
    lines.append(f'<rect x="0" y="0" width="{W}" height="36" rx="12" fill="{D["bg1"]}" opacity=".95"/>')
    lines.append(f'<rect x="0" y="24" width="{W}" height="12" fill="{D["bg1"]}" opacity=".95"/>')
    lines.append(f'<text x="16" y="23" font-size="13" font-weight="700" fill="{D["white"]}" '
                 f'font-family="Sora,sans-serif">{_svg_escape(title)}</text>')
    lines.append(f'<rect x="{W-110}" y="8" width="100" height="20" rx="10" fill="{mode_col}" opacity=".2"/>')
    lines.append(f'<rect x="{W-110}" y="8" width="100" height="20" rx="10" fill="none" '
                 f'stroke="{mode_col}" stroke-width="1"/>')
    lines.append(f'<text x="{W-60}" y="21" text-anchor="middle" font-size="9" font-weight="700" '
                 f'fill="{mode_col}" font-family="IBM Plex Mono,monospace">{_svg_escape(mode_lbl)}</text>')

    # ── Legend (bottom) ──────────────────────────────────────────────
    leg_y = H - 28
    lines.append(f'<rect x="0" y="{leg_y-6}" width="{W}" height="34" fill="{D["bg1"]}" opacity=".9" rx="0"/>')
    legend_items = [
        ("⬜", D["blue"],   "LLM/Model"),
        ("⬜", D["green"],  "Data Store"),
        ("⬜", D["amber"],  "Service"),
        ...
        ("⋯", D["red"],    "Trust Boundary"),
    ]
    lx = 16
    for sym, col, lbl in legend_items:
        if sym == "⬜":
            lines.append(f'<rect x="{lx}" y="{leg_y+2}" width="12" height="12" rx="3" fill="{col}" opacity=".8"/>')
        else:
            lines.append(f'<text x="{lx}" y="{leg_y+12}" font-size="10" fill="{col}" opacity=".9">{sym}</text>')
        lines.append(f'<text x="{lx+15}" y="{leg_y+12}" font-size="9" fill="{D["grey"]}" '
...
def _ws1_svg_config():
    """WS1: LLM-Powered Customer Support Chatbot"""
    return {
        "title": "WS1: ShopAssist AI — LLM Customer Chatbot",
        "width": 900, "height": 540,
        "zones_layout": [
            {"zone":0,"x":10,"y":45,"w":160,"h":490},
            {"zone":3,"x":180,"y":45,"w":220,"h":490},
            {"zone":5,"x":410,"y":45,"w":280,"h":490},
            {"zone":9,"x":700,"y":45,"w":190,"h":490},
        ],
        "trust_boundaries": [
            {"x1":400,"y1":45,"x2":400,"y2":535,"label":"App / Infra Boundary"},
            {"x1":690,"y1":45,"x2":690,"y2":535,"label":"Model Boundary"},
        ],
        "nodes": [
            {"id":"user",     "x":20,  "y":100,"w":140,"h":56,"color":"teal",  "icon":"👤","label":"End User","sublabel":"Browser/Mobile","shape":"rect","maestro":"L7","zone":0},
            {"id":"attacker", "x":20,  "y":220,"w":140,"h":56,"color":"red",   "icon":"🎭","label":"Adversary","sublabel":"Prompt Injector","shape":"rect","maestro":"L7","zone":0},
            {"id":"cdn",      "x":20,  "y":360,"w":140,"h":56,"color":"grey",  "icon":"🌐","label":"CDN/WAF","sublabel":"Cloudflare","shape":"rect","maestro":"L4","zone":0},
            {"id":"chat_ui",  "x":195, "y":100,"w":185,"h":56,"color":"blue",  "icon":"💬","label":"Chat UI","sublabel":"React Frontend","shape":"rect","maestro":"L5","zone":3},
            {"id":"api_gw",   "x":195, "y":220,"w":185,"h":56,"color":"amber", "icon":"🔀","label":"API Gateway","sublabel":"Auth + Rate Limit","shape":"rect","maestro":"L4","zone":3},
            {"id":"session_db","x":195,"y":350,"w":185,"h":56,"color":"green", "icon":"💾","label":"Session DB","sublabel":"Redis","shape":"cylinder","maestro":"L2","zone":3},
            {"id":"llm_svc",  "x":425, "y":100,"w":245,"h":56,"color":"purple","icon":"🤖","label":"LLM Service","sublabel":"System Prompt + Context","shape":"rect","maestro":"L5","zone":5},
            {"id":"prompt_eng","x":425,"y":220,"w":245,"h":56,"color":"blue",  "icon":"📝","label":"Prompt Engine","sublabel":"Template Assembly","shape":"rect","maestro":"L5","zone":5},
            {"id":"audit_log","x":425, "y":350,"w":245,"h":56,"color":"amber", "icon":"📋","label":"Audit Logger","sublabel":"Immutable Event Log","shape":"cylinder","maestro":"L6","zone":5},
            {"id":"llm_api",  "x":715, "y":100,"w":165,"h":56,"color":"red",   "icon":"🧠","label":"Foundation LLM","sublabel":"GPT-4 / Claude API","shape":"rect","maestro":"L1","zone":9},
            {"id":"api_key",  "x":715, "y":220,"w":165,"h":56,"color":"amber", "icon":"🔑","label":"API Key Store","sublabel":"AWS Secrets Manager","shape":"cylinder","maestro":"L4","zone":9},
            {"id":"monitor",  "x":715, "y":350,"w":165,"h":56,"color":"green", "icon":"📊","label":"Monitoring","sublabel":"LLM Guard / Alerts","shape":"rect","maestro":"L4","zone":9},
        ],
        "edges": [
...
def _ws2_svg_config():
    """WS2: RAG Legal AI Pipeline"""
    return {
        "title": "WS2: LegalAI — RAG Knowledge Pipeline",
        "width": 960, "height": 560,
        "zones_layout": [
            {"zone":0,"x":10,"y":45,"w":140,"h":510},
            {"zone":3,"x":160,"y":45,"w":200,"h":510},
            {"zone":5,"x":370,"y":45,"w":210,"h":510},
            {"zone":8,"x":590,"y":45,"w":200,"h":510},
            {"zone":9,"x":800,"y":45,"w":150,"h":510},
        ],
        "trust_boundaries": [
            {"x1":360,"y1":45,"x2":360,"y2":555,"label":"User / App Boundary"},
            {"x1":585,"y1":45,"x2":585,"y2":555,"label":"App / Data Boundary"},
            {"x1":795,"y1":45,"x2":795,"y2":555,"label":"Data / Model Boundary"},
        ],
        "nodes": [
            {"id":"lawyer",    "x":20, "y":100,"w":120,"h":54,"color":"teal",  "icon":"👩‍⚖️","label":"Lawyer","sublabel":"Authenticated","maestro":"L7","zone":0},
            {"id":"attacker",  "x":20, "y":220,"w":120,"h":54,"color":"red",   "icon":"🎭","label":"Attacker","sublabel":"Malicious Doc","maestro":"L7","zone":0},
            {"id":"doc_upload","x":170,"y":100,"w":180,"h":54,"color":"blue",  "icon":"📄","label":"Doc Upload","sublabel":"PDF/Word Ingest","maestro":"L5","zone":3},
            {"id":"ui",        "x":170,"y":220,"w":180,"h":54,"color":"teal",  "icon":"🖥️","label":"Query UI","sublabel":"Legal Q&A Interface","maestro":"L5","zone":3},
            {"id":"authz",     "x":170,"y":350,"w":180,"h":54,"color":"amber", "icon":"🔐","label":"AuthZ Engine","sublabel":"Client ACLs","maestro":"L4","zone":3},
            {"id":"rag_engine","x":380,"y":100,"w":185,"h":54,"color":"purple","icon":"🔍","label":"RAG Engine","sublabel":"Retrieval Orchestrator","maestro":"L3","zone":5},
            {"id":"llm",       "x":380,"y":220,"w":185,"h":54,"color":"red",   "icon":"🧠","label":"LLM Inference","sublabel":"Context + Generate","maestro":"L1","zone":5},
            {"id":"chunk_svc", "x":380,"y":350,"w":185,"h":54,"color":"blue",  "icon":"✂️","label":"Chunk Service","sublabel":"Text Splitter","maestro":"L2","zone":5},
            {"id":"vector_db", "x":600,"y":100,"w":180,"h":54,"color":"green", "icon":"🗃️","label":"Vector DB","sublabel":"Pinecone / Weaviate","shape":"cylinder","maestro":"L2","zone":8},
            {"id":"doc_store", "x":600,"y":220,"w":180,"h":54,"color":"amber", "icon":"📚","label":"Doc Store","sublabel":"Source Documents","shape":"cylinder","maestro":"L2","zone":8},
            {"id":"embed_svc", "x":600,"y":350,"w":180,"h":54,"color":"blue",  "icon":"🔢","label":"Embed Service","sublabel":"OpenAI Embeddings","maestro":"L2","zone":8},
            {"id":"model_wts", "x":810,"y":100,"w":135,"h":54,"color":"red",   "icon":"⚖️","label":"Model Weights","sublabel":"LLM Registry","shape":"cylinder","maestro":"L1","zone":9},
            {"id":"secrets",   "x":810,"y":220,"w":135,"h":54,"color":"purple","icon":"🔑","label":"Secrets","sublabel":"API Keys","shape":"cylinder","maestro":"L4","zone":9},
        ],
        "edges": [
...
def _ws3_svg_config():
    """WS3: MLOps Fraud Detection"""
    return {
        "title": "WS3: FraudShield — ML Model API & MLOps",
        "width": 980, "height": 560,
        "zones_layout": [
            {"zone":0,"x":10,"y":45,"w":145,"h":510},
            {"zone":3,"x":165,"y":45,"w":200,"h":510},
            {"zone":5,"x":375,"y":45,"w":210,"h":510},
            {"zone":8,"x":595,"y":45,"w":195,"h":510},
            {"zone":9,"x":800,"y":45,"w":170,"h":510},
        ],
        "trust_boundaries": [
            {"x1":370,"y1":45,"x2":370,"y2":555,"label":"App / Serving Boundary"},
            {"x1":590,"y1":45,"x2":590,"y2":555,"label":"Serving / Data Boundary"},
            {"x1":795,"y1":45,"x2":795,"y2":555,"label":"Data / Training Boundary"},
        ],
        "nodes": [
            {"id":"bank_app",  "x":18, "y":100,"w":130,"h":54,"color":"teal",  "icon":"🏦","label":"Bank App","sublabel":"Transaction API","maestro":"L7","zone":0},
            {"id":"attacker",  "x":18, "y":230,"w":130,"h":54,"color":"red",   "icon":"🎭","label":"Adversary","sublabel":"Sponge/Evasion","maestro":"L7","zone":0},
            {"id":"data_sci",  "x":18, "y":370,"w":130,"h":54,"color":"grey",  "icon":"👩‍💻","label":"Data Scientist","sublabel":"MLOps Engineer","maestro":"L7","zone":0},
            {"id":"api_gw",    "x":175,"y":100,"w":182,"h":54,"color":"amber", "icon":"🔀","label":"Model API GW","sublabel":"Auth + Rate Limit","maestro":"L4","zone":3},
            {"id":"feature_svc","x":175,"y":230,"w":182,"h":54,"color":"blue", "icon":"⚙️","label":"Feature Service","sublabel":"Real-time Features","maestro":"L2","zone":3},
            {"id":"cicd",      "x":175,"y":370,"w":182,"h":54,"color":"purple","icon":"🔁","label":"CI/CD Pipeline","sublabel":"Model Registry Push","maestro":"L4","zone":3},
            {"id":"infer_svc", "x":385,"y":100,"w":190,"h":54,"color":"red",   "icon":"🧮","label":"Inference Service","sublabel":"Model Serving (KServe)","maestro":"L4","zone":5},
            {"id":"fraud_mdl", "x":385,"y":230,"w":190,"h":54,"color":"purple","icon":"🤖","label":"Fraud Model","sublabel":"XGBoost / DNN","maestro":"L1","zone":5},
            {"id":"explain",   "x":385,"y":370,"w":190,"h":54,"color":"teal",  "icon":"📊","label":"Explainability","sublabel":"SHAP / LIME","maestro":"L6","zone":5},
            {"id":"train_data","x":605,"y":100,"w":175,"h":54,"color":"amber", "icon":"📦","label":"Training Data","sublabel":"Feature Store","shape":"cylinder","maestro":"L2","zone":8},
            {"id":"model_reg", "x":605,"y":230,"w":175,"h":54,"color":"green", "icon":"📋","label":"Model Registry","sublabel":"MLflow / W&B","shape":"cylinder","maestro":"L4","zone":8},
            {"id":"data_pipe", "x":605,"y":370,"w":175,"h":54,"color":"blue",  "icon":"🔄","label":"Data Pipeline","sublabel":"Airflow / Spark","maestro":"L2","zone":8},
            {"id":"raw_data",  "x":810,"y":100,"w":152,"h":54,"color":"red",   "icon":"🗄️","label":"Raw Txn Data","sublabel":"PII / PCI Data","shape":"cylinder","maestro":"L2","zone":9},
            {"id":"model_wts", "x":810,"y":230,"w":152,"h":54,"color":"purple","icon":"⚖️","label":"Model Weights","sublabel":"Versioned Artifacts","shape":"cylinder","maestro":"L1","zone":9},
            {"id":"secrets",   "x":810,"y":370,"w":152,"h":54,"color":"amber", "icon":"🔑","label":"Secrets Vault","sublabel":"API Keys","shape":"cylinder","maestro":"L4","zone":9},
        ],
        "edges": [
...
def _ws4_svg_config():
    """WS4: Autonomous Research Agent"""
    return {
        "title": "WS4: ResearchAgent — Autonomous Multi-Tool Agent",
        "width": 1000, "height": 580,
        "zones_layout": [
            {"zone":0,"x":10,"y":45,"w":140,"h":530},
            {"zone":3,"x":160,"y":45,"w":195,"h":530},
            {"zone":5,"x":365,"y":45,"w":220,"h":530},
            {"zone":7,"x":595,"y":45,"w":200,"h":530},
            {"zone":9,"x":805,"y":45,"w":185,"h":530},
        ],
        "trust_boundaries": [
            {"x1":358,"y1":45,"x2":358,"y2":575,"label":"User / Agent Boundary"},
            {"x1":590,"y1":45,"x2":590,"y2":575,"label":"Agent / Tool Boundary"},
            {"x1":800,"y1":45,"x2":800,"y2":575,"label":"Tool / Infra Boundary"},
        ],
        "nodes": [
            {"id":"researcher","x":18, "y":100,"w":126,"h":54,"color":"teal",  "icon":"🔬","label":"Researcher","sublabel":"Trusted User","maestro":"L7","zone":0},
            {"id":"web_attk",  "x":18, "y":240,"w":126,"h":54,"color":"red",   "icon":"🎭","label":"Web Attacker","sublabel":"Malicious Page","maestro":"L7","zone":0},
            {"id":"email_rec", "x":18, "y":380,"w":126,"h":54,"color":"grey",  "icon":"📧","label":"Email Recip.","sublabel":"External Target","maestro":"L7","zone":0},
            {"id":"chat_if",   "x":170,"y":100,"w":177,"h":54,"color":"blue",  "icon":"💬","label":"Chat Interface","sublabel":"Agent Orchestrator","maestro":"L5","zone":3},
            {"id":"task_plan", "x":170,"y":240,"w":177,"h":54,"color":"purple","icon":"🗺️","label":"Task Planner","sublabel":"ReAct / Chain-of-Thought","maestro":"L3","zone":3},
            {"id":"mem_store", "x":170,"y":380,"w":177,"h":54,"color":"amber", "icon":"🧠","label":"Agent Memory","sublabel":"Long-term + Working","shape":"cylinder","maestro":"L3","zone":3},
            {"id":"agent_exec","x":375,"y":100,"w":200,"h":54,"color":"red",   "icon":"⚡","label":"Agent Executor","sublabel":"Tool Dispatch Loop","maestro":"L3","zone":5},
            {"id":"llm_core",  "x":375,"y":240,"w":200,"h":54,"color":"purple","icon":"🧠","label":"LLM Core","sublabel":"Claude / GPT-4","maestro":"L1","zone":5},
            {"id":"tool_router","x":375,"y":380,"w":200,"h":54,"color":"blue", "icon":"🔀","label":"Tool Router","sublabel":"Permission Enforcer","maestro":"L3","zone":5},
            {"id":"web_tool",  "x":605,"y":80, "w":182,"h":50,"color":"teal",  "icon":"🌐","label":"Web Browser","sublabel":"Search + Scrape","maestro":"L3","zone":7},
            {"id":"code_tool", "x":605,"y":180,"w":182,"h":50,"color":"amber", "icon":"💻","label":"Code Executor","sublabel":"Sandbox Python/Bash","maestro":"L6","zone":7},
            {"id":"email_tool","x":605,"y":280,"w":182,"h":50,"color":"green", "icon":"📧","label":"Email Tool","sublabel":"Send / Read Email","maestro":"L6","zone":7},
            {"id":"file_tool", "x":605,"y":380,"w":182,"h":50,"color":"purple","icon":"📁","label":"File Manager","sublabel":"Read / Write Files","maestro":"L6","zone":7},
            {"id":"cal_tool",  "x":605,"y":470,"w":182,"h":50,"color":"blue",  "icon":"📅","label":"Calendar API","sublabel":"Schedule Events","maestro":"L6","zone":7},
            {"id":"sandbox",   "x":815,"y":90, "w":168,"h":54,"color":"red",   "icon":"🏖️","label":"Exec Sandbox","sublabel":"Container/VM","maestro":"L4","zone":9},
            {"id":"audit_trail","x":815,"y":210,"w":168,"h":54,"color":"green","icon":"📋","label":"Audit Trail","sublabel":"Immutable Action Log","shape":"cylinder","maestro":"L6","zone":9},
            {"id":"auth_svc",  "x":815,"y":350,"w":168,"h":54,"color":"amber", "icon":"🔐","label":"Auth Service","sublabel":"OAuth + Permissions","maestro":"L4","zone":9},
        ],
        "edges": [
...
# SESSION STATE
# ══════════════════════════════════════════════════════════════════════
SAVE_PATH = "/tmp/ai_tm_lab_v2.json"
...
# ══════════════════════════════════════════════════════════════════════
# INTERACTIVE DFD BUILDER DATA
# ══════════════════════════════════════════════════════════════════════
DFD_NODE_TYPES = {
    "LLM API": {"color":"red",   "shape":"rect",   "maestro":"L1","zone":9,"icon":"🧠","desc":"Foundation model (OpenAI, Anthropic, etc.)"},
    "Agent":   {"color":"purple","shape":"rect",   "maestro":"L3","zone":5,"icon":"🤖","desc":"Autonomous agent executor with tool access"},
    "Vector DB":{"color":"green","shape":"cylinder","maestro":"L2","zone":8,"icon":"🗃️","desc":"Embedding store for RAG retrieval"},
    ...
    "API Gateway":{"color":"amber","shape":"rect", "maestro":"L4","zone":3,"icon":"🔀","desc":"Auth + rate limiting layer"},
    "Database":{"color":"green","shape":"cylinder","maestro":"L2","zone":7,"icon":"💾","desc":"Structured data store"},
    "Tool":    {"color":"blue",  "shape":"rect",   "maestro":"L3","zone":5,"icon":"🔧","desc":"Agent tool (web, code, email, etc.)"},
...
def maestro_badge(layer_id):
    ...
    return (f'<span style="display:inline-flex;align-items:center;gap:5px;'
            f'padding:4px 12px;border-radius:6px;font-size:.78rem;font-weight:700;'
            f'font-family:IBM Plex Mono,monospace;background:rgba(0,0,0,.3);'
...
def owasp_badge(owasp_id):
    ...
    return (f'<span style="display:inline-flex;align-items:center;gap:4px;'
            f'padding:3px 10px;border-radius:5px;font-size:.76rem;font-weight:700;'
            f'font-family:IBM Plex Mono,monospace;background:rgba(0,0,0,.3);'
            f'border:1px solid {color};color:{color};">'
...
def ws_step_nav(ws_id, n_steps=7):
    ...
    for i, (col, name) in enumerate(zip(cols, step_names), 1):
        with col:
            if i < step:
                st.markdown(f'<div style="text-align:center;font-size:.78rem;color:{D["green"]};font-weight:700;">✓ {name}</div>', unsafe_allow_html=True)
            elif i == step:
                st.markdown(f'<div style="text-align:center;font-size:.78rem;color:{D["amber"]};font-weight:700;border-bottom:2px solid {D["amber"]};padding-bottom:2px;">● {name}</div>', unsafe_allow_html=True)
            else:
                st.markdown(f'<div style="text-align:center;font-size:.78rem;color:{D["grey2"]};font-weight:500;">○ {name}</div>', unsafe_allow_html=True)
...
def page_home():
    ...
    <div style="background:linear-gradient(135deg,{D['bg1']},{D['bg3']});
                border:1px solid {D['border']};border-radius:14px;
      ...
      <div style="position:absolute;top:0;right:0;width:300px;height:300px;
                  background:radial-gradient(circle,rgba(45,140,255,.15),transparent 70%);
      ...
      <h1 style="font-family:Sora,sans-serif;font-size:2.6rem;font-weight:800;
                 ...
                 margin:0 0 .5rem 0;">AI Threat Modeling Learning Lab</h1>
      <p style="font-size:1.1rem;color:{D['grey']};margin:.5rem 0 1.5rem 0;max-width:700px;">
        Master enterprise-grade threat modeling for AI systems — STRIDE · MAESTRO · OWASP LLM Top 10 · MITRE ATLAS.<br>
        Learn by doing with 4 real-world AI system scenarios and enterprise architecture diagrams.
      </p>
      <div style="display:flex;gap:12px;flex-wrap:wrap;">
        <span style="background:rgba(45,140,255,.2);border:1px solid {D['blue']};
                     color:{D['blue2']};padding:5px 14px;border-radius:20px;font-size:.85rem;font-weight:600;">
          🛡️ STRIDE Framework</span>
        <span style="background:rgba(139,92,246,.2);border:1px solid {D['purple']};
                     color:{D['purple2']};padding:5px 14px;border-radius:20px;font-size:.85rem;font-weight:600;">
          🤖 MAESTRO (Ken Huang / CSA)</span>
        <span style="background:rgba(16,201,143,.2);border:1px solid {D['green']};
                     color:{D['green']};padding:5px 14px;border-radius:20px;font-size:.85rem;font-weight:600;">
          📋 OWASP LLM Top 10 (2025)</span>
        <span style="background:rgba(245,166,35,.2);border:1px solid {D['amber']};
                     color:{D['amber2']};padding:5px 14px;border-radius:20px;font-size:.85rem;font-weight:600;">
          🔬 MITRE ATLAS</span>
      </div>
    </div>
    """, unsafe_allow_html=True)

    # How this differs from Ken Huang's tool
    with st.expander("📖 How this Lab differs from MAESTRO Sentinel (Ken Huang's tool)"):
        st.markdown(f"""
        <table style="width:100%;border-collapse:collapse;font-size:.9rem;">
        <thead><tr>
          <th style="background:{D['bg3']};color:{D['amber2']};padding:10px;border:1px solid {D['border']};text-align:left;">Feature</th>
          <th style="background:{D['bg3']};color:{D['amber2']};padding:10px;border:1px solid {D['border']};text-align:center;">MAESTRO Sentinel</th>
          <th style="background:{D['bg3']};color:{D['blue2']};padding:10px;border:1px solid {D['border']};text-align:center;">This Lab</th>
        </tr></thead>
        <tbody>
        {"".join(f'''<tr>
          <td style="padding:8px 10px;border:1px solid {D['border']};color:{D['white']};">{row[0]}</td>
          <td style="padding:8px 10px;border:1px solid {D['border']};text-align:center;color:{D['grey']};">{row[1]}</td>
          <td style="padding:8px 10px;border:1px solid {D['border']};text-align:center;color:{D['green']};">{row[2]}</td>
        </tr>''' for row in [
            ["Purpose", "Auto-generates threat models from architecture text", "Teaches HOW and WHY to threat model AI systems"],
            ["MAESTRO Framework", "✅ Full 7-layer analysis", "✅ Full 7-layer + agentic threat explanation"],
            ["Architecture Diagrams", "Text description input only", "✅ Enterprise SVG diagrams with MAESTRO badges + zone bands"],
            ["Interactive DFD Builder", "❌", "✅ Build your own architecture diagram"],
            ["Hands-on Assessment", "❌ (auto-generated output)", "✅ Scored exercises per threat with feedback"],
            ["OWASP LLM Top 10", "Partial (in mitigations)", "✅ Every threat mapped with specific controls"],
            ["STRIDE Integration", "Referenced", "✅ Zone-direction rules + AI-specific STRIDE mapping"],
            ["MITRE ATLAS", "❌", "✅ ATT&CK for ML techniques per threat"],
            ["Attack Trees", "❌", "✅ Step-by-step attack path analysis"],
            ["Progress/Certification", "❌", "✅ 8-hour program with scored certificate"],
            ["Why Agentic Threats Differ", "States threats exist", "✅ Explains autonomy + non-determinism + no trust boundary"],
        ])}
        </tbody></table>
        """, unsafe_allow_html=True)

    # Progress summary
    st.markdown("## Workshop Progress")
    ...
    cols = st.columns(2)
    ws_list = [("1","2"), ("3","4")]
    for row_pair in ws_list:
        for i, ws_id in enumerate(row_pair):
            ws = WORKSHOPS[ws_id]
            ...
            step      = st.session_state["ws_step"].get(ws_id, 1)
            focus_layers = " ".join([f'<span style="background:{D.get(l,D["grey"])}22;border:1px solid {D.get(l,D["grey"])}55;color:{D.get(l,D["grey"])};padding:2px 7px;border-radius:4px;font-size:.72rem;font-weight:700;">{l}</span>' for l in ws["maestro_focus"]])
            status_badge = (f'<span style="background:{D["green"]}22;color:{D["green"]};border:1px solid {D["green"]}44;padding:3px 10px;border-radius:12px;font-size:.75rem;font-weight:700;">✓ Complete</span>' if complete
                           else f'<span style="background:{D["blue"]}22;color:{D["blue2"]};border:1px solid {D["blue"]}44;padding:3px 10px;border-radius:12px;font-size:.75rem;font-weight:700;">Step {step}/7</span>' if unlocked and step > 1
                           else f'<span style="background:{D["grey2"]}44;color:{D["grey"]};border:1px solid {D["grey2"]};padding:3px 10px;border-radius:12px;font-size:.75rem;font-weight:700;">🔒 Locked</span>' if not unlocked
                           else f'<span style="background:{D["amber"]}22;color:{D["amber2"]};border:1px solid {D["amber"]}44;padding:3px 10px;border-radius:12px;font-size:.75rem;font-weight:700;">Ready</span>')
            col_idx = int(ws_id) % 2 - 1 if int(ws_id) % 2 != 0 else 1
            with cols[col_idx if ws_id in ["1","3"] else 1-col_idx if ws_id in ["2","4"] else 0]:
                border_col = ws["color"] if unlocked else D["grey2"]
                bg_opacity = "18" if unlocked else "08"
                st.markdown(f"""
                <div style="background:{ws['color']}{bg_opacity};border:1px solid {border_col}44;
                            border-radius:12px;padding:1.3rem;margin:.4rem 0;
                            {'opacity:.5;' if not unlocked else ''}">
                  <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:.6rem;">
                    <div>
                      <span style="font-size:1.6rem;">{ws['icon']}</span>
                      <span style="font-family:Sora,sans-serif;font-size:1rem;font-weight:700;
                                   color:{ws['color']};margin-left:.4rem;">WS{ws_id}: {ws['title']}</span>
                    </div>
                    {status_badge}
                  </div>
                  <div style="color:{D['white']};font-size:.9rem;font-weight:600;margin-bottom:.3rem;">{ws['subtitle']}</div>
                  <div style="color:{D['grey']};font-size:.82rem;margin-bottom:.6rem;">
                    {ws['level']} · {ws['duration']} · 5 threat scenarios</div>
                  <div style="margin-bottom:.7rem;">{focus_layers}</div>
                  {'<div style="background:rgba(0,0,0,.2);border-radius:6px;height:6px;"><div style="background:'+ws['color']+';height:6px;border-radius:6px;width:'+str(int(score/50*100))+'%;"></div></div>' if score > 0 else ''}
                  {'<div style="font-size:.8rem;color:'+D['grey']+';margin-top:4px;">Score: '+str(score)+'/50</div>' if score > 0 else ''}
                </div>
                """, unsafe_allow_html=True)
                if unlocked:
                    ...
                    with st.form(f"unlock_{ws_id}"):
                        code = st.text_input("Unlock code:", placeholder="Enter workshop code", key=f"code_{ws_id}", label_visibility="collapsed")
    ...
    with tab1:
        st.markdown("### MAESTRO 7-Layer Reference Architecture (Ken Huang / CSA)")
        alert("MAESTRO models AI agent systems as a 7-layer stack. Each layer has distinct threat categories: **Traditional** (apply to any software at that layer) and **Agentic** (novel threats arising from non-determinism, autonomy, and no-trust-boundary).", "maestro")
        for lid, layer in MAESTRO_LAYERS.items():
            with st.expander(f"{layer['icon']} {lid}: {layer['name']} — Zone {layer['zone']}"):
                ...
                with cols[0]:
                    ...
                    for t in layer["traditional"]:
                        st.markdown(f"• {t}")
                    st.markdown("**Agentic Threats (Non-determinism / Autonomy / No Trust Boundary):**")
                    for t in layer["agentic"]:
                        st.markdown(f"• ⚠️ {t}")
                with cols[1]:
                    st.markdown("**STRIDE:**")
                    for s in layer["stride"]:
                        st.markdown(f'<span style="color:{STRIDE_INFO.get(s,{}).get("color",D["grey"])};">● {s}</span>', unsafe_allow_html=True)
                    st.markdown("**OWASP LLM:**")
                    for o in layer["owasp"]:
                        st.markdown(f'<span style="color:{OWASP_LLM.get(o,{}).get("color",D["grey"])};">● {o}: {OWASP_LLM.get(o,{}).get("name","")}</span>', unsafe_allow_html=True)
                    st.markdown("**MITRE ATLAS:**")
                    for m in layer["mitre"]:
                        st.markdown(f"• `{m}`")
    ...
    with tab3:
        ...
        for stride, info in STRIDE_INFO.items():
            ...
            with col1:
                st.markdown(f'<div style="background:{info["color"]}22;border:1px solid {info["color"]}55;border-radius:8px;padding:.7rem;text-align:center;color:{info["color"]};font-weight:700;">{info["icon"]}<br>{stride}</div>', unsafe_allow_html=True)
            with col2:
                st.markdown(f"**In AI Systems:** {info['ai']}")

    with tab4:
        st.markdown("### AI System Zone Model")
        st.markdown("Zones represent trust levels. Threats cross zone boundaries. Higher zone number = higher trust = more protected = more valuable to attackers.")
        for zn, zi in ZONES.items():
            st.markdown(f'<div style="background:{zi["fill"]};border:1px solid {zi["stroke"]};border-radius:8px;padding:.7rem;margin:.3rem 0;"><b style="color:{zi["color"]};">z{zn} — {zi["name"]}</b></div>', unsafe_allow_html=True)
...
def page_workshop():
    ...
    with col_title:
        ...
        st.markdown(f"<span style='color:{D['grey']};'>{ws['subtitle']} · {ws['level']} · {ws['duration']}</span>", unsafe_allow_html=True)
...
def ws_step_design(ws_id, ws):
    ...
    alert("""<b>Infosec 4-Step Method — Step 1: Design</b><br>
Before identifying threats, we must understand the system. A Data Flow Diagram (DFD) shows:
<br>• <b>Interactors</b> — people and external systems that interact with the system
<br>• <b>Modules / Processes</b> — components that process data (including AI models, agents, pipelines)
<br>• <b>Data Stores</b> — where data persists (vector DBs, training datasets, model registries)
<br>• <b>Data Flows</b> — how data moves between components (these are where threats travel)
<br><br><b>For AI systems:</b> add MAESTRO layer labels to every component. This tells us what type of AI threat to look for at each component.""", "info")

    st.markdown("### System Scenario")
    panel(f'<b style="color:{ws["color"]};">{ws["title"]}</b><br><br>{ws["scenario"]}', "blue")
    ...
    st.caption("Enterprise-grade architecture diagram with MAESTRO layer badges and trust zones")
    ...
    node_data = []
    for n in cfg["nodes"]:
        maestro_id = n.get("maestro","")
        layer = MAESTRO_LAYERS.get(maestro_id, {})
        node_data.append({
            "Component": n["label"],
            "Type": n.get("sublabel",""),
            "MAESTRO Layer": f"{maestro_id}: {layer.get('name','')}",
            "Zone": f"z{n.get('zone',0)}",
            "Shape": n.get("shape","rect").title(),
        })
...
def ws_step_zones(ws_id, ws):
    ...
    alert("""<b>Infosec 4-Step Method — Step 2: Assign Zones of Trust</b><br>
Zones (0-9) represent how much we trust a component and how critical it is to protect.<br>
<b>Key rule:</b> A threat exists when data flows from a LOWER trust zone to a HIGHER trust zone — or when a high-trust component is accessed from a low-trust source.
<br><br><b>For AI systems:</b> Foundation model weights and training data sit in z9 (maximum security). External users are z0. The LLM API is z5 (elevated trust — high value, must be protected).
""", "info")

    st.markdown("### Zone Reference")
    cols = st.columns(3)
    for i, (zn, zi) in enumerate(ZONES.items()):
        with cols[i % 3]:
            st.markdown(f'<div style="background:{zi["fill"]};border:1px solid {zi["stroke"]};border-radius:8px;padding:.7rem;margin:.2rem 0;">'
                        f'<b style="color:{zi["color"]};">z{zn}: {zi["name"]}</b></div>', unsafe_allow_html=True)

    st.markdown("### Annotated Architecture — Zone View")
    svg = render_architecture_svg(ws_id, mode="zones")
    st.markdown(f'<div style="overflow-x:auto;">{svg}</div>', unsafe_allow_html=True)

    st.markdown("### Zone Assignment Exercise")
    alert("For each component below, identify the correct zone assignment and explain why the zone-crossing creates a threat surface.", "warn")
    ...
    for n in cfg["nodes"]:
        z = n.get("zone",0)
        zi = ZONES.get(z, ZONES[0])
        node_data.append({
            "Component": n["label"],
            "Zone": f"z{z}",
            "Trust Level": zi["name"],
            "Why this zone": _zone_rationale(n["label"], z, n.get("maestro","")),
        })
    st.dataframe(pd.DataFrame(node_data), use_container_width=True, hide_index=True)

    st.markdown("### Critical Zone Crossings")
    st.markdown("These are the zone crossings with the highest threat potential in this architecture:")
...
def _zone_rationale(label, zone, maestro):
    rationales = {
        9: "Maximum security — core AI model weights, training data. Stolen or tampered = system fully compromised.",
        8: "Critical — operational data stores (vector DBs, feature stores). Breached = persistent data corruption.",
        7: "High trust — agent tools with external access. Compromised = bridge to exfiltration.",
        5: "Elevated — LLM API, inference services. High value; must be shielded from z0 direct access.",
        4: "Standard app — application services with data access. Normal enterprise application tier.",
        3: "Minimal trust — API gateways, frontend, CI/CD. Entry points; must authenticate and sanitize.",
        1: "Low trust — partner/integration tier with limited access.",
        0: "External / untrusted — users, internet, attackers. Assume adversarial.",
    }
    return rationales.get(zone, f"Zone {zone}")

# ── Step 3: MAESTRO Analysis ───────────────────────────────────────────────
def ws_step_maestro(ws_id, ws):
    st.markdown("## Step 3: MAESTRO Layer Analysis")
    alert("""<b>Infosec 4-Step Method — Step 3: Discover Threats with MAESTRO + STRIDE</b><br>
MAESTRO analyzes threats at each architectural layer. For AI systems, every layer has both:
<br>• <b>Traditional threats</b> — security risks that apply to any software at that layer
<br>• <b>Agentic threats</b> — novel threats from <i>non-determinism</i>, <i>autonomy</i>, and <i>no trust boundary</i>
<br><br>The three agentic AI threat amplifiers are:
<br>🎲 <b>Non-Determinism</b>: Same input → different outputs. Threats may succeed intermittently, evading testing.
<br>🤖 <b>Autonomy</b>: Agent acts without human approval. A single injection can cause irreversible real-world harm.
<br>🚫 <b>No Trust Boundary</b>: AI components trust their context without verifying the caller's authorization.""", "maestro")

    focus_layers = ws["maestro_focus"]
    st.markdown(f"### MAESTRO Layers Featured in WS{ws_id}: {ws['title']}")
    for lid in focus_layers:
        ...
        with st.expander(f"{layer['icon']} {lid}: {layer['name']} — Zone {layer['zone']}", expanded=True):
            ...
            with col1:
                st.markdown(f'<div class="alert alert-info"><b>Traditional Threats at {lid}:</b><br>'
                            ...
                            unsafe_allow_html=True)
            with col2:
                st.markdown(f'<div class="alert alert-maestro"><b>⚠️ Agentic Threats at {lid} — Why They Are Worse:</b><br>'
            ...
            with cols2[0]:
                ...
                for s in layer["stride"]:
                    info = STRIDE_INFO.get(s,{})
                    st.markdown(f'<span style="color:{info.get("color",D["grey"])};">{info.get("icon","")} {s}</span>', unsafe_allow_html=True)
            with cols2[1]:
                st.markdown("**OWASP LLM:**")
                for o in layer["owasp"]:
                    item = OWASP_LLM.get(o,{})
                    st.markdown(f'<span style="color:{item.get("color",D["grey"])};">{o}: {item.get("name","")}</span>', unsafe_allow_html=True)
            with cols2[2]:
                st.markdown("**MITRE ATLAS:**")
                for m in layer["mitre"]:
                    st.markdown(f"`{m}`")

    st.markdown("### STRIDE → OWASP LLM Quick Mapping")
    st.markdown("For AI systems, each STRIDE category maps to specific OWASP LLM vulnerabilities:")
    for stride, sinfo in STRIDE_INFO.items():
        col1, col2, col3 = st.columns([2,3,3])
        with col1:
            st.markdown(f'<span style="color:{sinfo["color"]};font-weight:700;">{sinfo["icon"]} {stride}</span>', unsafe_allow_html=True)
        with col2:
            st.markdown(sinfo["ai"])
        with col3:
            for oid in [o for o, item in OWASP_LLM.items() if stride in item["stride"]]:
                item = OWASP_LLM[oid]
                st.markdown(f'<span style="color:{item["color"]};font-size:.85rem;">{oid}: {item["name"]}</span>', unsafe_allow_html=True)
...
For AI systems, the key insight is: <b>the 'easy' path is almost always through the AI input/output interface</b>, 
not through network intrusion. Direct prompt injection is cheaper than SQL injection. Poisoning a 
public dataset is cheaper than compromising a data center.""", "warn")

    # Show threat attack paths
    st.markdown("### Attack Paths for This Workshop")
    threats = ws["threats"]
    for t in threats:
        with st.expander(f"🌲 Attack Tree: {t['title']} ({t['stride']})"):
            col1, col2 = st.columns([3,2])
            with col1:
                st.markdown(f"**Goal:** Exploit — {t['title']}")
                st.markdown("**Attack path (sequential):**")
                for i, step in enumerate(t["attack_path"], 1):
                    col_a, col_b = st.columns([1,9])
                    with col_a:
                        st.markdown(f'<div style="background:{D["bg3"]};border:1px solid {D["border"]};border-radius:50%;width:28px;height:28px;display:flex;align-items:center;justify-content:center;font-weight:700;color:{D["amber"]};font-size:.9rem;">{i}</div>', unsafe_allow_html=True)
                    with col_b:
                        st.markdown(step)
                    if i < len(t["attack_path"]):
                        st.markdown(f'<div style="margin-left:14px;width:2px;height:16px;background:{D["border"]};"></div>', unsafe_allow_html=True)
            with col2:
                ...
                zi_from = ZONES.get(t["zone_from"], ZONES[0])
                zi_to   = ZONES.get(t["zone_to"],   ZONES[0])
                st.markdown(f'**Zone crossing:** <span style="color:{zi_from["color"]}">z{t["zone_from"]}</span> → <span style="color:{zi_to["color"]}">z{t["zone_to"]}</span>', unsafe_allow_html=True)
                likelihood_colors = {1:D["grey"],2:D["green"],3:D["amber"],4:D["amber"],5:D["red"]}
                st.markdown(f'**Likelihood:** <span style="color:{likelihood_colors.get(t["likelihood"],D["grey"])}">{"●"*t["likelihood"]}{"○"*(5-t["likelihood"])}</span>', unsafe_allow_html=True)
                impact_colors = {1:D["grey"],2:D["green"],3:D["amber"],4:D["amber"],5:D["red"]}
                st.markdown(f'**Impact:** <span style="color:{impact_colors.get(t["impact"],D["grey"])}">{"●"*t["impact"]}{"○"*(5-t["impact"])}</span>', unsafe_allow_html=True)
                risk = t["likelihood"] * t["impact"]
                risk_col = D["red"] if risk>=15 else D["amber"] if risk>=8 else D["green"]
                st.markdown(f'**Risk Score:** <span style="color:{risk_col};font-weight:700;">{risk}/25</span>', unsafe_allow_html=True)

    # Risk matrix
    st.markdown("### Risk Matrix")
    matrix_data = {t["title"]: {"Likelihood": t["likelihood"], "Impact": t["impact"],
                                 "Risk Score": t["likelihood"]*t["impact"], "STRIDE": t["stride"],
                                 "OWASP": t["owasp"]} for t in threats}
    df_risk = pd.DataFrame(matrix_data).T.sort_values("Risk Score", ascending=False)
    ...
    with col2:
        if st.button("→ Next: Identify & Log Threats", key=f"next4_{ws_id}", type="primary"):
            _advance_step(ws_id)

# ── Step 5: Identify Threats (Scored Exercise) ─────────────────────────────
def ws_step_identify(ws_id, ws):
    st.markdown("## Step 5: Identify & Log Threats (Scored Exercise)")
    alert("""<b>Hands-On Lab:</b> For each threat scenario, analyze the threat and select the correct STRIDE category, 
MAESTRO layer, and mitigations. You earn points for correct answers.<br><br>
<b>Scoring:</b> STRIDE (2pt) + MAESTRO layer (2pt) + likelihood/impact (1pt each) + mitigations (3pt) − wrong mitigations (−1pt each) = 10 pts per threat""", "warn")
    ...
    for idx, threat in enumerate(threats, 1):
        ...
        border_col = D["red"] if threat["likelihood"]*threat["impact"] >= 15 else D["amber"] if threat["likelihood"]*threat["impact"] >= 8 else D["green"]
        st.markdown(f"""
        <div style="background:{D['bg2']};border:1px solid {border_col}55;border-radius:12px;
                    padding:1.2rem;margin:1rem 0;border-left:4px solid {border_col};">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:.5rem;">
            <span style="font-family:Sora,sans-serif;font-size:1.05rem;font-weight:700;color:{D['white']};">
              {idx}. {threat['title']}</span>
            {'<span style="color:'+D["green"]+';font-weight:700;">✓ Scored: '+str(earned)+'/10</span>' if submitted else '<span style="color:'+D["amber"]+';">⏳ Not yet submitted</span>'}
          </div>
          <p style="color:{D['grey']};font-size:.88rem;line-height:1.6;margin:0;">{threat['description']}</p>
        </div>
        """, unsafe_allow_html=True)

        if not submitted:
            with st.form(key=f"threat_form_{ws_id}_{threat['id']}"):
                ...
                with fc4:
                    sel_likelihood = st.slider("Likelihood (1=rare, 5=certain):", 1, 5, 3, key=f"l_{ws_id}_{idx}")
                with fc5:
                    sel_impact = st.slider("Impact (1=low, 5=critical):", 1, 5, 3, key=f"i_{ws_id}_{idx}")

                all_mitigations = threat["correct_mitigations"] + threat["incorrect_mitigations"]
                import random; random.seed(hash(threat["id"]))
                random.shuffle(all_mitigations)
                sel_mitigations = st.multiselect("Select applicable mitigations:", all_mitigations, key=f"mit_{ws_id}_{idx}")

                if st.form_submit_button(f"Submit Analysis for Threat {idx}", type="primary"):
                    sc = 0
                    if sel_stride   == threat["stride"]:  sc += 2
                    if sel_maestro  == threat["maestro"]:  sc += 2
                    if abs(sel_likelihood - threat["likelihood"]) <= 1: sc += 1
                    if abs(sel_impact - threat["impact"]) <= 1:         sc += 1
                    correct_sel = [m for m in sel_mitigations if m in threat["correct_mitigations"]]
                    wrong_sel   = [m for m in sel_mitigations if m in threat["incorrect_mitigations"]]
                    mit_score = min(3, len(correct_sel)) - len(wrong_sel)
                    sc += max(0, mit_score)
                    sc = max(0, sc)
                    st.session_state[score_key] = sc
            ...
            with st.expander(f"📚 Full Analysis: {threat['title']}"):
                col1, col2 = st.columns(2)
                with col1:
                    st.markdown(f"**Why this is {threat['stride']}:**")
                    st.markdown(threat["why_stride"])
                    st.markdown(f"**Why MAESTRO {threat['maestro']}:**")
                    st.markdown(threat["why_maestro"])
                with col2:
                    st.markdown("**Why agentic systems make this worse:**")
                    st.markdown(threat["why_agentic"])
                    st.markdown("**Real-world examples:**")
                    st.markdown(threat["real_world"])
                st.markdown("**Correct mitigations:**")
                for m in threat["correct_mitigations"]:
                    st.markdown(f"✅ {m}")
                st.markdown("**Incorrect / insufficient mitigations:**")
                for m in threat["incorrect_mitigations"]:
                    st.markdown(f"❌ {m}")

    # Running total
    st.markdown(f"""
    <div style="background:{D['bg3']};border:1px solid {D['border']};border-radius:10px;
                padding:1rem;text-align:center;margin-top:1rem;">
      <span style="font-size:1.1rem;font-weight:700;color:{D['white']};">Running Score: </span>
      <span style="font-size:1.4rem;font-weight:800;color:{D['blue2']};">{total_score}/{max_score}</span>
...
def ws_step_assess(ws_id, ws):
    ...
    st.markdown(f'<div class="score-box score-{grade}" style="margin-bottom:1rem;">'
                f'{ws["icon"]} WS{ws_id} Score: {score}/50 — {label}</div>', unsafe_allow_html=True)
    ...
    st.markdown("### Per-Threat Score Breakdown")
    breakdown = []
    for t in threats:
    ...
    # Methodology review
    st.markdown("### 4-Step Methodology Review")
    tab1,tab2,tab3,tab4 = st.tabs(["1. Design","2. Zones","3. MAESTRO+STRIDE","4. Controls"])
    with tab1:
        st.markdown("**What we designed:**")
        st.markdown(f"Architecture: {ws['subtitle']} — {len(threats)} threat surfaces identified")
        svg = render_architecture_svg(ws_id, "architecture")
        st.markdown(f'<div style="overflow-x:auto;max-height:300px;">{svg}</div>', unsafe_allow_html=True)
    with tab2:
        st.markdown("**Key zone crossings identified:**")
        for t in threats:
            zi_from = ZONES.get(t["zone_from"], ZONES[0]); zi_to = ZONES.get(t["zone_to"], ZONES[0])
            st.markdown(f"• **{t['title']}**: z{t['zone_from']} ({zi_from['name']}) → z{t['zone_to']} ({zi_to['name']})")
    with tab3:
        st.markdown("**MAESTRO layers and STRIDE categories applied:**")
        for t in threats:
            layer = MAESTRO_LAYERS.get(t["maestro"],{})
            owasp = OWASP_LLM.get(t["owasp"],{})
            st.markdown(f"• **{t['title']}**: {t['stride']} @ {t['maestro']} ({layer.get('name','')}) → {t['owasp']}: {owasp.get('name','')}")
    with tab4:
        st.markdown("**Controls identified per threat:**")
        for t in threats:
            st.markdown(f"**{t['title']}:**")
            for m in t["correct_mitigations"][:2]:
                st.markdown(f"  ✅ {m}")
...
def ws_step_complete(ws_id, ws):
    ...
    st.markdown(f"""
    <div style="background:linear-gradient(135deg,{D['bg1']},{D['bg3']});
                border:2px solid {ws['color']}55;border-radius:16px;
                padding:2.5rem;text-align:center;margin:1rem 0;">
      <div style="font-size:3rem;margin-bottom:.5rem;">{ws['icon']}</div>
      <h2 style="color:{ws['color']};font-size:1.8rem;margin:.3rem 0;">WS{ws_id}: {ws['title']}</h2>
      <p style="color:{D['grey']};margin:.3rem 0;">{ws['subtitle']}</p>
      <div style="font-size:3rem;font-weight:900;color:{color};margin:1rem 0;">{score}/50</div>
      <div style="font-size:1.2rem;font-weight:700;color:{D['white']};margin-bottom:1rem;">{label}</div>
      <div style="display:inline-block;background:{color}22;border:2px solid {color};
                  border-radius:12px;padding:.8rem 2rem;font-weight:800;color:{color};font-size:1.1rem;">
    ...
    st.markdown(f"""
    In this workshop you analyzed a **{ws['subtitle']}** using:
    - **STRIDE** to categorize threats by type
    - **MAESTRO** layers ({', '.join(ws['maestro_focus'])}) to identify AI-specific attack surfaces  
    - **OWASP LLM Top 10** to select evidence-based controls
    - **Zone analysis** to understand where trust boundaries create attack paths
    - **Attack trees** to model realistic attacker paths
    """)
    ...
    if next_ws in WORKSHOPS and not st.session_state["unlocked"].get(next_ws):
        ...
        next_ws_data = WORKSHOPS[next_ws]
        alert(f"Ready for **WS{next_ws}: {next_ws_data['title']}** ({next_ws_data['level']})? Use the unlock code from your instructor.", "success")
        with st.form(f"unlock_next_{ws_id}"):
            code_next = st.text_input(f"Enter code for WS{next_ws}:", placeholder="Workshop unlock code")
            if st.form_submit_button(f"Unlock WS{next_ws}"):
                if code_next.strip().upper() == WORKSHOP_CODES[next_ws]:
                    ...
                    _save_state()
                    st.success(f"✅ Workshop {next_ws} unlocked!")
                    st.rerun()
                else:
                    st.error("Incorrect code")
    ...
    with col1:
        if st.button("← Redo This Workshop", key=f"redo_{ws_id}"):
    ...
    with col3:
        if next_ws in WORKSHOPS and st.session_state["unlocked"].get(next_ws):
            if st.button(f"▶ Start WS{next_ws}", key=f"next_ws_{ws_id}", type="primary"):
                st.session_state["page"] = "workshop"
                st.session_state["active_ws"] = next_ws
                st.session_state["ws_step"][next_ws] = 1
                st.rerun()
...
def page_dfd_builder():
    ...
    st.markdown("Build your own AI system architecture diagram. Add components, assign MAESTRO layers and zones, then generate your threat model.")

    alert("This builder generates enterprise-grade SVG diagrams. Add nodes and connections to build your DFD, then export as SVG or generate a threat model.", "info")
    ...
    with col_left:
        ...
        with st.form("add_node"):
            ...
            node_zone   = st.selectbox("Zone:", [f"z{z} — {ZONES[z]['name']}" for z in sorted(ZONES.keys())])
            ...
            if st.form_submit_button("➕ Add Component"):
                ...
                new_node = {"id": f"node_{len(st.session_state['dfd_nodes'])+1}",
                            "label": node_label or node_type, "x": node_x, "y": node_y,
                            "w": 140, "h": 54, "color": nt["color"],
                            "shape": nt["shape"], "maestro": node_maestro,
                            "zone": zone_n, "icon": nt["icon"],
                            "sublabel": node_maestro}
                st.session_state["dfd_nodes"].append(new_node)
                st.rerun()
        ...
        if st.button("🔄 Clear All", key="clear_dfd"):
            st.session_state["dfd_nodes"] = []; st.session_state["dfd_edges"] = []; st.rerun()
    ...
    with col_right:
        ...
        if nodes:
            ...
            W, H = 900, 500
            custom_cfg = {
                "title": "Custom AI Architecture — My Design",
                "width": W, "height": H,
                "nodes": nodes, "edges": edges,
                "zones_layout": [],
                "trust_boundaries": [],
            }
            mode_c = st.radio("View:", ["architecture","zones","threat"], horizontal=True, key="dfd_mode")
            svg = _build_svg(custom_cfg, mode_c, [n["id"] for n in nodes if n.get("zone",0) >= 7])
            st.markdown(f'<div style="overflow-x:auto;">{svg}</div>', unsafe_allow_html=True)

            # SVG download
            b64_svg = base64.b64encode(svg.encode()).decode()
            st.markdown(f'<a href="data:image/svg+xml;base64,{b64_svg}" download="architecture_diagram.svg" '
                        f'style="background:{D["blue"]};color:white;padding:.5rem 1rem;border-radius:6px;'
                        f'text-decoration:none;font-weight:600;">📥 Download SVG</a>', unsafe_allow_html=True)

            # Auto-generate threat model
            if len(nodes) >= 3:
                st.markdown("### Auto-Generated Threat Assessment")
                alert("Based on your MAESTRO layer assignments and zone crossings, here are the most likely threat categories for your architecture:", "maestro")
            ...
            <div style="background:{D['bg3']};border:2px dashed {D['border']};
                        border-radius:12px;padding:3rem;text-align:center;color:{D['grey']};">
              <div style="font-size:2rem;margin-bottom:1rem;">🎨</div>
              <p>Add components on the left to build your architecture diagram.</p>
              <p style="font-size:.85rem;">Start with: User → API Gateway → LLM Service → Foundation Model</p>
            </div>""", unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        <div style="background:linear-gradient(135deg,{D['bg2']},{D['bg3']});
          ...
          <div style="font-family:Sora,sans-serif;font-weight:700;font-size:1rem;
                      background:linear-gradient(90deg,{D['blue2']},{D['amber']});
                      -webkit-background-clip:text;-webkit-text-fill-color:transparent;">
            AI Threat Modeling Lab</div>
          <div style="font-size:.75rem;color:{D['grey']};margin-top:.2rem;">
        ...
        if st.button("🏠 Dashboard", key="nav_home"):
            st.session_state["page"] = "home"; st.rerun()
        if st.button("🎨 DFD Builder", key="nav_dfd"):
            st.session_state["page"] = "dfd"; st.rerun()
        ...
        for ws_id, ws in WORKSHOPS.items():
            ...
            label_col = D["green"] if complete else (D["amber"] if unlocked else D["grey2"])
            score_str = f" ({score}/50)" if score > 0 else ""
            if st.button(f"{icon_str} WS{ws_id}: {ws['title']}{score_str}", key=f"nav_ws{ws_id}",
        ...
        with st.expander("MAESTRO Layers"):
            for lid, layer in MAESTRO_LAYERS.items():
                st.markdown(f'<span style="color:{layer["color"]};">**{lid}** {layer["icon"]}</span> {layer["name"]} (z{layer["zone"]})', unsafe_allow_html=True)

        with st.expander("OWASP LLM Top 10"):
            for oid, item in OWASP_LLM.items():
                st.markdown(f'<span style="color:{item["color"]};font-size:.85rem;">**{oid}**: {item["name"]}</span>', unsafe_allow_html=True)

        with st.expander("Zone Scores"):
        ...
        st.markdown("---")
        st.markdown(f'<div style="font-size:.75rem;color:{D["grey"]};text-align:center;">'
                    f'Based on Ken Huang\'s MAESTRO Framework<br>'
                    f'Cloud Security Alliance · OWASP LLM Top 10 (2025)<br>'
                    f'MITRE ATLAS · STRIDE</div>', unsafe_allow_html=True)
